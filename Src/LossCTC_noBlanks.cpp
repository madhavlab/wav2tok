...
std::tuple<Tensor, Tensor> ctc_loss_cpu_template(const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths, int64_t BLANK) {
  ...
  at::parallel_for(0, batch_size, 0, [&](int64_t start, int64_t end) {
    for (const auto b : c10::irange(start, end)) {
      ...
      if (target_length > 0)
        log_alpha_a[0][0] = log_probs_a[0][get_target_prime(targets_data, tg_batch_offset, tg_target_stride, 0, BLANK)];

      // now the loop over the inputs
      for (const auto t : c10::irange(1, input_length)) {
        for (const auto s : c10::irange(1,target_length)) {
      ...
      if (target_length == 0) {
        ...
        scalar_t l1 = neginf //log_alpha_a[input_length-1][target_length];
        scalar_t l2 = log_alpha_a[input_length-1][target_length];
  ...
  at::parallel_for(0, batch_size, 0, [&](int64_t start, int64_t end) {
    ...
    for (const auto b : c10::irange(start, end)) {
      ...
      if (input_length > 0) {
        ...
        if (target_length > 0) {
          auto current_target_prime = get_target_prime(targets_data, tg_batch_offset, tg_target_stride, target_length, BLANK);
          log_beta_a[input_length-1][target_length] = log_probs_a[input_length-1][current_target_prime];

          // the first two are a blank and a non-blank, so we know they are different and we don't need to do log+
          grad_a[input_length-1][current_target_prime] = log_alpha_a[input_length-1][target_length] + log_beta_a[input_length-1][target_length];
        }
      }
      ...
      for (int64_t t=input_length-2; t>=0; t--) {
        ...
        // alternatively, one might consider moving s to the outer loop to cache current_target_prime more (but then it needs to be descending)
        // for the cuda implementation, that gave a speed boost.
        for (int64_t s=target_length; s>=0; s--) {
